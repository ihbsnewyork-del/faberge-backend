...
exports.deleteNotificationByAdmin = exports.deleteTransactionByAdmin = exports.deleteBookingByAdmin = exports.updateBookingStatus = exports.getMonthlyRevenue = exports.getMonthlyEarnings = exports.getAllNotifications = exports.getAllTransactions = exports.getAllBookings = exports.getBookingTrends = exports.getWorkerPopularity = exports.getCustomerBookings = exports.getWorkerMonthlyCalendar = exports.getWorkerBookings = exports.bookTimeSlot = exports.startCleanupScheduler = exports.cleanupExpiredBookings = exports.handleStripeWebhook = exports.initializePayment = void 0;
...
const timeSlot_controller_1 = __importDefault(require("../timeSlot/timeSlot.controller"));
...
const calculateTotalPrice = async (booking) => {
    ...
    for (const srv of booking.services) {
        const serviceDoc = await services_model_1.ServiceModel.findById(srv.service);
        if (!serviceDoc) {
        ...
        if (srv.subcategories && srv.subcategories.length > 0) {
            for (const subId of srv.subcategories) {
                const sub = serviceDoc.subcategory?.find((s) => s._id.toString() === subId.toString());
                if (sub) {
...
const initializePayment = async (req, res) => {
    try {
        ...
        const customer = await customer_model_1.CustomerModel.findById(customerId);
        if (!customer) {
        ...
        const worker = await worker_model_1.WorkerModel.findById(workerId);
        if (!worker) {
...
const handleSuccessfulPayment = async (session) => {
    try {
        ...
        const booking = await booking_model_1.BookingModel.findByIdAndUpdate(bookingId, {
            isPayment: true,
        ...
        const timeSlotDoc = await timeSlot_model_1.TimeSlotModel.findOne({
            worker: booking.worker,
            date: booking.date,
        });
        if (timeSlotDoc) {
            const slot = timeSlotDoc.slots.find((s) => s.startTime === booking.startTime);
            if (slot) {
        ...
        await transporter.sendMail({
            ...
            text: `Your booking has been confirmed. Booking ID: ${bookingId}. Customer: ${customer.firstName} ${customer.lastName}. Date: ${booking.date}. Time: ${booking.startTime}. Location: ${customer.address}, ${customer.city}, ${customer.state} ${customer.zipCode ? ", " + customer.zipCode : ""}.`,
        });
        await transporter.sendMail({
            from: process.env.EMAIL_USER,
            to: customer.email,
...
const cleanupExpiredBookings = async () => {
    try {
        ...
        for (const booking of expiredBookings) {
            ...
            if (timeSlotDoc) {
                const slotIndex = timeSlotDoc.slots.findIndex((s) => s.startTime === booking.startTime);
                if (slotIndex !== -1) {
                    ...
                    if (prevSlot)
                        prevSlot.isBlocked = false;
                    if (nextSlot)
                        nextSlot.isBlocked = false;
                    await timeSlotDoc.save();
                    console.log(`✅ Released slot for booking: ${booking._id}`);
                }
            }
        }
        console.log(`✅ Cleanup completed: ${expiredBookings.length} bookings expired`);
    }
    catch (error) {
...
const startCleanupScheduler = () => {
    setInterval(exports.cleanupExpiredBookings, 60 * 1000);
...
const bookTimeSlot = async (req, res) => {
    try {
        ...
        const worker = await worker_model_1.WorkerModel.findById(workerId).populate("services");
        if (!worker) {
            res.status(404).json({ message: "Worker not found" });
            return;
        }
        const workerServiceIds = worker.services.map((s) => s.service.toString());
        ...
        let timeSlotDoc = await timeSlot_model_1.TimeSlotModel.findOne({ worker: workerId, date });
        if (!timeSlotDoc) {
            timeSlotDoc = new timeSlot_model_1.TimeSlotModel({
                worker: workerId,
                date,
                slots: (0, timeSlot_controller_1.default)(),
                heldBy: customer?._id,
            });
            await timeSlotDoc.save();
        }
        ...
        const slotIndex = timeSlotDoc.slots.findIndex((s) => s.startTime === convertedStartTime);
        if (slotIndex === -1) {
        ...
        if (slot.heldUntil && now >= slot.heldUntil && !slot.isBooked) {
            ...
            const prevSlot = timeSlotDoc.slots[slotIndex - 1];
            const nextSlot = timeSlotDoc.slots[slotIndex + 1];
            if (prevSlot && prevSlot.isBlocked)
                prevSlot.isBlocked = false;
            if (nextSlot && nextSlot.isBlocked)
                nextSlot.isBlocked = false;
            await timeSlotDoc.save();
            if (slot.heldBy) {
                await booking_model_1.BookingModel.findByIdAndUpdate(slot.heldBy, {
                    status: "expired",
                });
            }
        }
        if (slot.heldUntil && now < slot.heldUntil) {
            const remainingMinutes = Math.ceil((slot.heldUntil.getTime() - now.getTime()) / (1000 * 60));
            res.status(400).json({
        ...
        const nextSlot = timeSlotDoc.slots[slotIndex + 1];
        if (prevSlot)
            prevSlot.isBlocked = true;
        if (nextSlot)
            nextSlot.isBlocked = true;
        await timeSlotDoc.save();
        console.log(`✅ Slot temporarily reserved until: ${paymentExpiresAt}`);
        res.status(201).json({
            message: "Slot temporarily reserved. Please complete payment within 3 minutes.",
            data: booking,
...
// --------------------
const getWorkerMonthlyCalendar = async (req, res) => {
    try {
        ...
        const timeSlots = await timeSlot_model_1.TimeSlotModel.find({
            worker: workerId,
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (currentDate < today) {
                ...
                if (timeSlot.isOffDay) {
                    ...
                    const bookedSlots = timeSlot.slots.filter((s) => s.isBooked).length;
            ...
            calendarData.push({
                ...
                availableSlots: timeSlot
                    ? timeSlot.slots.filter((s) => s.isAvailable && !s.isBooked && !s.isBlocked).length
                    : (0, timeSlot_controller_1.default)().length,
            });
        }
        res.status(200).json({
...
exports.getWorkerMonthlyCalendar = getWorkerMonthlyCalendar;
// --------------------
...
// --------------------
const getWorkerPopularity = async (req, res) => {
    try {
        const month = parseInt(req.query.month);
        const year = parseInt(req.query.year);
        if (!month || !year) {
        ...
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0, 23, 59, 59);
        const popularity = await booking_model_1.BookingModel.aggregate([
            {
                $match: {
                    date: { $gte: startDate, $lte: endDate },
                },
...
exports.getWorkerPopularity = getWorkerPopularity;
// --------------------
// Get Bookings Trends
// --------------------
const getBookingTrends = async (req, res) => {
    try {
        ...
        const finalData = monthNames.map((name, index) => {
            ...
            const found = result.find((r) => r._id === monthNumber);
            return {
                month: name,
...
const getAllBookings = async (req, res) => {
    try {
        ...
        // ========================================================
        const result = await (0, paginationHelper_1.paginate)(booking_model_1.BookingModel, {
            page: Number(page) || 1,
            limit: Number(limit) || 10,
            sort: { createdAt: -1 },
            filter,
        });
        let populatedData = await booking_model_1.BookingModel.populate(result.data, [
            {
                path: "customer",
                select: "firstName lastName email phone uploadPhoto address city state zipCode",
            },
            {
                path: "worker",
        ...
        populatedData = populatedData.map((booking) => {
            const bookingObj = booking.toObject ? booking.toObject() : booking;
            bookingObj.services = bookingObj.services.map((srv) => {
                const serviceDoc = srv.service;
                if (!serviceDoc)
                    return srv;
                const selectedSubcategories = (srv.subcategories || [])
                    .map((subId) => (serviceDoc.subcategory || []).find((sub) => sub._id.toString() === subId.toString()))
                    .filter(Boolean);
                return {
                    _id: srv._id,
                    service: {
...
const getAllTransactions = async (req, res) => {
    try {
        ...
        const populatedData = await booking_model_1.BookingModel.populate(result.data, [
...
const getMonthlyRevenue = async (req, res) => {
    try {
        ...
        const revenueData = await booking_model_1.BookingModel.aggregate([
            {
                $match: {
                    isPayment: true,
                    paymentAmount: { $gt: 0 },
...
const deleteBookingByAdmin = async (req, res) => {
    try {
        ...
        //   }
        // }
        await booking_model_1.BookingModel.findByIdAndDelete(bookingId);
        res.status(200).json({
...
const deleteTransactionByAdmin = async (req, res) => {
    try {
        ...
        const booking = await booking_model_1.BookingModel.findById(transactionId);
        if (!booking) {
            res.status(404).json({ message: "Booking not found" });
            return;
        }
...
exports.deleteTransactionByAdmin = deleteTransactionByAdmin;
const deleteNotificationByAdmin = async (req, res) => {
    try {
        const { notificationId } = req.params;
        console.log(notificationId);
        const booking = await booking_model_1.BookingModel.findById(notificationId);
        if (!booking) {
            res.status(404).json({ message: "Booking not found" });
            return;
        }
...
exports.deleteNotificationByAdmin = deleteNotificationByAdmin;
