...
exports.getWorkerAvailability = exports.getAllWorkerAvailability = exports.setOffDay = exports.setWorkerUnAvailability = void 0;
...
exports.default = generateDefaultSlots;
// ---------------------------------------
// Set Worker Unavailability
// ---------------------------------------
const setWorkerUnAvailability = async (req, res) => {
    try {
        ...
        let timeSlot = await timeSlot_model_1.TimeSlotModel.findOne({ worker: workerId, date });
        if (timeSlot?.isOffDay === true) {
            res.status(400).json({ message: "Worker is off day" });
            return;
        }
        if (!timeSlot) {
            timeSlot = new timeSlot_model_1.TimeSlotModel({
                worker: workerId,
                date,
...
exports.setWorkerUnAvailability = setWorkerUnAvailability;
// -------------------------
// Set OffDay
// -------------------------
const setOffDay = async (req, res) => {
    try {
        const workerId = req.user.userId;
        const { date } = req.body;
        if (!date) {
        ...
        const timeSlot = await timeSlot_model_1.TimeSlotModel.findOne({
            worker: workerId,
            date,
        });
        const startOfDay = new Date(`${date}T00:00:00.000Z`);
        const endOfDay = new Date(`${date}T23:59:59.999Z`);
        const bookingExists = await booking_model_1.BookingModel.findOne({
            worker: workerId,
        ...
        if (bookingExists) {
            res.status(400).json({
                message: "You cannot set this day as off day. A booking already exists.",
            });
            return;
        }
        if (timeSlot && timeSlot.isOffDay) {
            const offDay = await timeSlot_model_1.TimeSlotModel.findOneAndUpdate({ worker: workerId, date: startOfDay }, {
                worker: workerId,
                date: startOfDay,
                isOffDay: false,
                slots: generateDefaultSlots(),
        ...
        const offDay = await timeSlot_model_1.TimeSlotModel.findOneAndUpdate({ worker: workerId, date: startOfDay }, { worker: workerId, date: startOfDay, isOffDay: true, slots: [] }, { upsert: true, new: true });
        res.status(200).json({
...
exports.setOffDay = setOffDay;
// -------------------------
// Get All Workers Availability (Paginated)
// -------------------------
const getAllWorkerAvailability = async (req, res) => {
    try {
        const page = Number(req.query.page) || 1;
        const limit = Number(req.query.limit) || 10;
        const skip = (page - 1) * limit;
        const total = await timeSlot_model_1.TimeSlotModel.countDocuments();
...
exports.getAllWorkerAvailability = getAllWorkerAvailability;
// -------------------------
...
const getWorkerAvailability = async (req, res) => {
    try {
        ...
        let timeSlot = await timeSlot_model_1.TimeSlotModel.findOne({
            worker: workerId,
            date,
        });
        if (!timeSlot ||
        ...
        const today = new Date();
        const isToday = requestedDate.getFullYear() === today.getFullYear() &&
